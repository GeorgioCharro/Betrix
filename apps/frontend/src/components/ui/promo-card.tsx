...
const PromoCard = React.forwardRef<HTMLDivElement, PromoCardProps>(
  ({ imageSrc, title, icon: Icon, className, navigateTo, ...props }, ref) => {
    ...
    return (
      <Card
        className={cn(
          'cursor-pointer w-[320px] h-[200px] flex flex-col justify-between overflow-hidden transition-transform duration-200 hover:-translate-y-1',
          className
        )}
        onClick={handleClick}
        ref={ref}
        {...props}
      >
        <img alt={title} className="w-full h-[160px] object-cover" src={imageSrc} />
